...
public class VampiricStrikeSO : SpellSO
{
    ...
    {
        int halfOfAtkPwr = Mathf.RoundToInt(ctx.Stats.GetStat(StatType.AttackPower)/2);
        if (ctx.EnemyHealthSystem == null)
        {
            Debug.LogWarning("Vampiric strike tried to damage null enemy, this should not trigger!");
            return;
        }
        ctx.EnemyHealthSystem.TakeDamage(ctx.Stats.GetStat(StatType.AttackPower) + halfOfAtkPwr);
